    ...
    fn encode_statement_assign_aggregate(
        ...
        match aggregate_kind {
            mir::AggregateKind::Adt(adt_did, variant_index, substs, _, active_field_index) => {
                ...
                let encoded_adt_def =
                //  if adt_def.is_enum() {
                //     self.encoder.encode_adt_def(adt_def, substs, Some(*variant_index))?
                // } else {
                //     assert!(adt_def.is_struct());
                //     assert_eq!(variant_index.index(), 0, "variant for structs should be 0");
                    self.encoder.encode_adt_def(adt_def, substs, None)?;
                // };
                match encoded_adt_def {
                    ...
                    vir_high::TypeDecl::Enum(decl) => {
                        ...
                        let variant_type = encoded_target
                            .get_type()
                            .clone()
                            .variant(variant_name.clone().into());
                        let encoded_target =
                            encoded_target.variant_no_pos(variant_name.into(), variant_type);
                        // FIXME: Refactor to remove duplication with the struct variant.
                        assert_eq!(variant.fields.len(), operands.len());
                        for (field, operand) in variant.fields.into_iter().zip(operands.iter()) {
                            let encoded_target_with_field =
                                encoded_target.clone().field_no_pos(field);
                            self.encode_assign_operand(
                                block_builder,
    ...
    fn encode_terminator(
        ...
        let successor = match &terminator.kind {
            ...
            TerminatorKind::Unreachable => {
                ...
                block_builder.add_comment(format!("Target marked as unreachable by the compiler"));
