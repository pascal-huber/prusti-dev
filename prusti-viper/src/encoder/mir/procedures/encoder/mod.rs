...
impl<'p, 'v: 'p, 'tcx: 'v> ProcedureEncoder<'p, 'v, 'tcx> {
    ...
    fn encode_close_mut_ref(
            ...
            base.get_type()
        {
            block_builder.add_statement(self.set_statement_error(
                ...
                vir_high::Statement::close_mut_ref_no_pos(lifetime.clone(), self.rd_perm, object),
            )?);
        };
        Ok(())
    }
    ...
    fn encode_open_mut_ref(
            ...
            base.get_type()
        {
            block_builder.add_statement(self.set_statement_error(
                location,
                ErrorCtxt::CloseMutRef,
                vir_high::Statement::open_mut_ref_no_pos(lifetime.clone(), self.rd_perm, object),
            )?);
        };
        Ok(())
    }
    ...
    fn encode_assign_operand(
        ...
        if let Some(base) = &deref_base {
            self.encode_open_mut_ref(block_builder, location, base, encoded_target.clone())?;
        }
        ...
        match operand {
            ...
            mir::Operand::Copy(source) => {
                ...
                if let Some(base) = &deref_base {
                    self.encode_close_mut_ref(block_builder, location, base, encoded_source)?;
                }
            }
        ...
        if let Some(base) = &deref_base {
            self.encode_close_mut_ref(block_builder, location, base, encoded_target)?;
        }
