    ...
    fn into_low<'p, 'v: 'p, 'tcx: 'v>(
        ...
        match self {
            ...
            Self::MovePlace(statement) => {
                ...
                assert_eq!(
                    target_ty.clone().erase_lifetime(),
                    source_ty.clone().erase_lifetime()
                );
            ...
            Self::Dead(statement) => {
                ...
                Ok(vec![Statement::comment(format!("Dead({})", statement))])
            }
            Self::LifetimeTake(statement) => {
                ...
                lowerer.encode_lft_tok_sep_return_method(2)?;
                lowerer.encode_lft_tok_sep_return_method(3)?;
